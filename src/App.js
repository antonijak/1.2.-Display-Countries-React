...
// defines if user wants starting word or any and sets the expression in the header aswell
  startingOrAny = (e) => { 
    ...
    this.setState({ typeOfSearch: 'first', startContain: 'starting with', startsButtonClass: 'isClicked', anyButtonClass: 'buttons'}) 
    } else {
    this.setState({ typeOfSearch: 'any', startContain: 'that contain', startsButtonClass: 'buttons', anyButtonClass: 'isClicked'}) 
    }
  }
...
// fills logical grammar to the verbs in the header p
  verbs = () => {
    if(this.findMatch().length === 1 && this.state.typeOfSearch === 'first'){
